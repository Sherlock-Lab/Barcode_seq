...
for key in multiplex_dic:
	...
	#  cluster BC1 (low diversity) to the known list and then try Leveihnstein distance to cluster them.

	data=open(directory+'/table_BC1_bartender.txt','r')
	tuto=[]
	splitou=[]
	while True:
		lignes_data=data.readline()
		if lignes_data=='':
			break
		oupf=lignes_data.split(',')
		tuto.append(oupf[0])
		#splitou.append(oupf)
	data.close()
	...
	x=set(tuto)
	dico_BC2={s:0 for s in x}
	for i in range(len(tuto)):
		dico_BC2[tuto[i]]+=1

	tuto=[]
	list_BC2=sorted([[s,dico_BC2[s]] for s in dico_BC2.keys()],key=itemgetter(1),reverse=True)
	lito=[list_BC2[i][0] for i in range(len(list_BC2))]
	list_BC2=[]
	#poubaya2=sorted(list_BC2,key=itemgetter(1),reverse=True)



	poubaya2=[s for s in x]
	poubaya=[poubaya2[i] for i in range(len(poubaya2)) if poubaya2[i] not in pool]
	setoo=set(poubaya)
	print(len(poubaya2))
	print(len(poubaya))
	real_dico_pooled={}
	poubaya_pool=[lito[i] for i in range(len(lito)) if lito[i] in pool]
	lito=[]
	for i in range(len(poubaya)):
		for j in range(len(poubaya_pool)):
			if levenshtein(poubaya_pool[j],poubaya[i])<min_distance:
				real_dico_pooled[poubaya[i]]=poubaya_pool[j]
				break

	seto=setoo-set(real_dico_pooled.keys())
	print(len(seto))
	for s in seto:
		real_dico_pooled[s]=s
	for s in poubaya_pool:
		real_dico_pooled[s]=s
	print(len(set(real_dico_pooled.values())))
	print(len(poubaya_pool))
	dapa=open(directory+'/table_BC1_clust.txt','w')
	data=open(directory+'/table_BC1_bartender.txt','r')
	while True:
		lignes_data=data.readline()
		if lignes_data=='':
	...
	#for i in range(len(splitou)):
		LD_def=real_dico_pooled[oupf[0]]
		dapa.write(LD_def+','+oupf[1])
	dapa.close()
	data.close()
	...
	cluster_BC2 = {}
	for line_BC2 in clusterBC2_file:  
	#loop over the real cluster for BC2 using the id of the cluster as key and the cluster center or "real BC" as value
		(cluster_id_BC2,BC2_clust,Score,point) = line_BC2.strip().split(',')
		if cluster_id_BC2.isdigit():
			cluster_BC2[cluster_id_BC2] = BC2_clust
	clusterBC2_file.close()
	
	#loop to cluster the BC2 that have different sizes TOTDO	
	group2 = {}

	for line in open("%s/clust_BC2_barcode.csv" %(directory),'r'):
	# output of bartender gives the uniques BC, b, their frequency, f, and the cluster they belong c_id.
	#  make a dict with the original bc and the clister_id from Bartender
	...
	bc1_handle = open("%s/table_BC1_clust.txt" % directory,'r')
	bc2_handle = open("%s/table_BC2_bartender.txt" % directory,'r')
	umi_handle = open('%s/table_UMI.txt' % directory,'r')

	for bc1 in bc1_handle:

		# go through clustered BC1 file and the 2 original files at the same time containing the same lines and associate the cluster id and umi to each reads/line
		# make a dict for each cluster based on the merged bc and put all the umi in a list as value
		...
		bc2 = cluster_BC2[id2]
		merged_bc = bc1  + '_' + bc2
		if merged_bc in merged_umis:
			merged_umis[merged_bc].append(umi)
		else:
			merged_umis[merged_bc] = [umi]
	...
	for key_id in merged_id_umi_dic:
		# reassociate the BC1 and teh BC2 together and keep all the 2BC that have more than 5 different UMI
		
		if float(merged_id_umi_dic[key_id])/after_pcr > 3./(depth):
			#  calculate frequency to eliminate low frequency lineages based on coverage depth
			BC1_id, BC2_id = key_id.split('_')

			w_cluster =  BC2_id + '\t' + BC1_id + '\t' + str(merged_id_umi_dic[key_id]) + str('\n')
