...
for line in multiplex_file:
	...
	outFile = open('%s/table_file.txt' % dir_name, 'w')
	...
	while True:
		...
		try:
			...
			if F_R_multi in outfile_dic: # and len(barcode_r) < 33 and len(barcode_f) < 33:
				...
				sample_file = outfile_dic[F_R_multi]
				count_line += 1
...
for fileHandle in outfile_dic: #close all those files
	outfile_dic[fileHandle].close()
...
for key in multiplex_dic:
	...
	for key_id in merged_id_umi_dic:
		...
		if int(merged_id_umi_dic[key_id]) > 5:
